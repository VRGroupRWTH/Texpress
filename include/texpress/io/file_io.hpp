...
namespace texpress {
  ...
  bool TEXPRESS_EXPORT file_exists(const char* path);
  uint64_t TEXPRESS_EXPORT file_size(const char* path);
  bool TEXPRESS_EXPORT file_read(const char* path, char* buffer, uint64_t buffer_size, FileType type = FileType::FILE_BINARY);
  bool TEXPRESS_EXPORT file_save(const char* path, char* buffer, uint64_t buffer_size, FileType type = FileType::FILE_BINARY);
